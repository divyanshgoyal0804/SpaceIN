...
import { Property, useChatStore } from '@/store/chatStore';
...
export default function PropertyResultCard({ property }: Props) {
  ...
  const handleViewClick = () => {
    trackEvent('CLICK', property.id, 'view_details');
  };

  const handleCallClick = () => {
    trackEvent('CONTACT', property.id, 'call');
  };
  ...
  return (
    <div className={styles.propertyCard}>
      ...
      <div className={styles.cardBody}>
        ...
        <Link href={detailHref} className={styles.viewBtn} onClick={handleViewClick}>View Details →</Link>
      </div>
    </div>
  );
}
